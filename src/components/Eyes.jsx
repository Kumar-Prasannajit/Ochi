import React from 'react'

function Eyes() {
  return (
    <div className='Eyes w-full h-screen overflow-hidden'>
        <div className='relative w-full h-full bg-cover bg-center bg-[url("https://ochi.design/wp-content/uploads/2022/05/Top-Viewbbcbv-1-scaled.jpg")]'>
            <div className='absolute left-1/2 top-1/2 -translate-x-[50%] -translate-y-[50%] flex gap-5'>
                <div className='w-[15vw] h-[15vw] rounded-full bg-zinc-100 flex items-center justify-center'>
                    <div className='w-2/3 h-2/3 rounded-full bg-zinc-900 relative'>
                        <div className='line w-full h-8 absolute top-1/2 left-1/2 -translate-x-[50%] -translate-y-[50%]'>
                            <div className='w-8 h-8 rounded-full bg-zinc-100'>
                        
                            </div> 
                        </div>
                    </div>
                </div>
                <div className='w-[15vw] h-[15vw] rounded-full bg-zinc-10               0 flex items-center justify-center'>
                    <div className='w-2/3 h-2/3 rounded-full bg-zinc-900 relative'>
                        <div className='line w-full h-8 absolute top-1/2 left-1/2 -translate-x-[50%] -translate-y-[50%]'>
                            <div className='w-8 h-8 rounded-full bg-zinc-100'>
                        
                            </div> 
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
  )
}

export default Eyes